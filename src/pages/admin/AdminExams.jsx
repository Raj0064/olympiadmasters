      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mt-2.5">
          ...
          {exam.batchNames?.length > 0 ? (
            exam.batchNames.map((name) => (
              <Badge key={name} variant="neutral">{name}</Badge>
            ))
          ) : (
            <Badge variant="neutral">No batch</Badge>
          )}
...
export default function AdminExams() {
  ...
  async function load() {
    setLoading(true);
    try {
      // ✅ Fetch exams + batches in parallel
      const [rawExams, batches] = await Promise.all([getExams(), getBatches()]);

      // ✅ Build reverse map: examId → [batchName, ...]
      // Batches own exams via examIds[], not the other way around
      const examToBatches = {};
      for (const batch of batches) {
        for (const examId of batch.examIds ?? []) {
          if (!examToBatches[examId]) examToBatches[examId] = [];
          examToBatches[examId].push(batch.name);
        }
      }

      // ✅ Attach resolved batch names to each exam
      const enriched = rawExams.map((e) => ({
        ...e,
        batchNames: examToBatches[e.id] ?? [], // e.g. ["Batch A", "Batch B"]
      }));
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-3 mb-6 sm:mb-8">
        <div>
          <h2 className="text-xl sm:text-2xl font-semibold text-text-dark">Exams</h2>
          <p className="text-sm text-text-muted mt-1">Create and manage exams</p>
        </div>
        <Button
          variant="primary"
          size="sm"
      ...
      {!loading && exams.length === 0 && (
        <EmptyState
          ...
          action={
            <Button variant="ghost" size="sm" onClick={() => navigate('/admin/exams/create')}>
          ...
          <div className="hidden md:block">
            <Card className="p-6">
              <h3 className="text-sm font-semibold text-text-dark mb-4">
                All Exams
                <span className="ml-2 text-text-faint font-normal">({exams.length})</span>
              </h3>
                    ...
                    {exams.map((e) => (
                      <tr key={e.id} className="hover:bg-black/[0.02] transition-colors">
                        <td className="py-4 pr-4 font-medium text-text-dark">
                          <span className="max-w-[250px] truncate block">{e.title}</span>
                        </td>
                        <td className="py-4 pr-4">
                          <Badge variant="info">Gr {e.grade}</Badge>
                        </td>

                        {/* ✅ Batch names resolved from batches collection */}
                        <td className="py-4 pr-4">
                          {e.batchNames.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {e.batchNames.map((name) => (
                                <Badge key={name} variant="neutral">{name}</Badge>
                              ))}
                            </div>
                          ) : (
                        ...
                        <td className="py-4 pr-0">
                          <div className="flex items-center gap-4">
                            ...
                            <button
                              onClick={() => navigate(`/admin/exams/${e.id}/submissions`)}
                              className="text-xs font-medium text-text-muted hover:text-text-dark hover:underline transition-colors"
                            >
                              Submissions
                            </button>
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                navigate(`/leaderboard?type=exam&examId=${e.id}`)
                              }
                            >
          ...
          <div className="md:hidden space-y-3.5">
            ...
            {exams.map((e) => (
              <ExamMobileCard
                ...
                onSubmissions={(id) => navigate(`/admin/exams/${id}/submissions`)}
              />
            ))}
          </div>
        </>
      ...
      <ConfirmDialog
        ...
        description={
          <>
            "<span className="font-semibold text-text-dark">{deleteTarget?.title}</span>"
            and all its questions will be permanently deleted. This cannot be undone.
          </>
        }
