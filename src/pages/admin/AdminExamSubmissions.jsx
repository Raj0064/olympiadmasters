...
function SubmissionCard({ sub, onView, onDelete }) {
  return (
    <Card className="p-3.5 space-y-2.5">
      {/* Student info */}
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-[13px] font-semibold text-text-dark leading-snug">
            {sub._studentName}
          </p>
  ...
  async function load() {
    ...
    try {
      ...
      const enriched = await Promise.all(
        subs.map(async (sub) => {
          try {
            const student = await getStudent(sub.userId);
            return {
              ...sub,
    ...
    <div>
      ...
      {error && (
        <div className="mb-3 flex justify-between items-center text-xs text-danger bg-danger-bg border border-danger/20 rounded-lg px-3 py-2">
          ...
          <button
            onClick={() => setError('')}
            className="ml-2 text-danger/50 hover:text-danger font-bold text-sm"
          >
            ×
          </button>
        </div>
              ...
              renderRow={(sub, idx) => (
                ...
                >
                  ...
                  <td className="py-2.5 pr-4">
                    <p className="text-[13px] font-medium text-text-dark leading-snug">
                      {sub._studentName}
                    </p>
                    <p className="text-[11px] text-text-faint">
                      {sub._studentEmail}
                  ...
                  <td className="py-2.5 pr-4 text-xs text-success font-medium">
                    {sub.correct ?? '—'}
                  </td>

                  <td className="py-2.5 pr-4 text-xs text-danger font-medium">
                    {sub.wrong ?? '—'}
                  </td>
                  ...
                  <td className="py-2.5 pr-4 text-xs text-text-muted whitespace-nowrap">
                  ...
                  <td className="py-2.5">
                    <div className="flex items-center gap-3 whitespace-nowrap">
                      <button
                        onClick={() =>
                          navigate(
                            `/admin/exams/${examId}/submissions/${sub.id}`
                          )
                        }
                        className="text-xs font-medium text-accent hover:underline"
                      >
          ...
          <div className="md:hidden space-y-2.5">
            ...
            {submissions.map((sub) => (
              <SubmissionCard
                ...
                sub={sub}
                onView={(id) =>
                  navigate(`/admin/exams/${examId}/submissions/${id}`)
                }
