...
const selectCls = `
  w-full border border-border rounded-xl px-3 py-2 text-sm text-text-dark
...
function TagInput({ tags, onChange, placeholder = "Type & press Enter…" }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5 p-2 min-h-[38px] bg-surface border border-border rounded-lg focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/10 transition-all duration-150">
      {tags.map((t) => (
        <span
          key={t}
          className="flex items-center gap-1 text-xs font-medium bg-indigo-bg text-indigo px-2 py-0.5 rounded-full"
        >
          {t}
          <button
            type="button"
  ...
  return (
    <div>
      ...
      <button
        ...
        disabled={uploading}
        className="mt-1.5 text-xs font-medium text-text-muted hover:text-accent flex items-center gap-1.5 transition-colors duration-150 disabled:opacity-40 disabled:cursor-not-allowed"
      >
        {uploading ? (
          <>
      ...
      {imageUrl && (
        <div className="mt-2 relative w-fit">
          <img
            src={imageUrl}
            alt=""
            className={`${sizeClass} rounded-xl border border-border object-contain shadow-sm transition-all duration-200`}
          />
          <button
            type="button"
            onClick={onRemove}
            className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-danger text-white text-xs flex items-center justify-center shadow-md hover:scale-110 transition-transform"
          >
            ×
          </button>
        </div>
        ...
        {SIZES.map((size) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1.5 rounded-xl border-2 py-2 px-1 transition-all duration-150 ${
                active
...
// ─── TopicSubtopicSelect ───────────────────────────────────────────────────────
// Used inside QuestionForm (per-question) — loads subtopics on demand.
function TopicSubtopicSelect({ topicId, subtopicId, topics, onChange }) {
  ...
  console.log("TopicSubtopicSelect - topics length:", topics.length);

  if (!topics.length) {
    return (
      <div className="text-xs text-text-muted italic p-2 bg-background rounded-xl">
        No topics available. Create topics for this grade first.
      </div>
    );
  }
  ...
  const handleSubtopicChange = (e) => {
    const id = e.target.value;
    const name = subtopics.find((s) => s.id === id)?.name || "";
    const topicName = topics.find((t) => t.id === topicId)?.name || "";
    onChange({ topicId, topicName, subtopicId: id, subtopicName: name });
  };
...
function QuestionForm({
  ...
  const setOpt = (k, v) =>
    setQ((p) => ({ ...p, options: { ...p.options, [k]: v } }));
  ...
  return (
    <div className="border border-accent/20 rounded-2xl p-5 bg-gradient-to-b from-accent/3 to-transparent space-y-5 mt-2 shadow-sm">
      {imgErr && (
      ...
      <fieldset className="space-y-2">
        ...
        <textarea
          ref={questionRef}
          className="w-full border border-border rounded-xl px-3 py-2.5 text-sm leading-6 text-text-dark outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 resize-none bg-surface transition-all duration-150 placeholder:text-text-faint"
        ...
        <ImageAttach
          ...
          onUpload={(f, folder) =>
            handleImgUpload(f, folder, "imageUrl", "imageUploading")
          }
          onRemove={() =>
            setQ((p) => ({ ...p, imageUrl: "", imageSize: "medium" }))
          }
        />
        {q.imageUrl && (
          <ImageSizePicker
            value={q.imageSize}
            onChange={(val) => set("imageSize", val)}
          />
        )}
      </fieldset>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {OPTIONS.map((opt) => {
            ...
            return (
              <div
                key={opt}
                className={`flex items-center gap-2.5 border rounded-xl px-3 py-2 transition-all duration-150 ${
                  isCorrect
              ...
              >
                <button
                  ...
                  className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center text-[11px] font-bold transition-all duration-150 ${
                    isCorrect
                ...
                <input
                  className={`flex-1 text-sm outline-none bg-transparent min-w-0 placeholder:text-text-faint ${
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            min="0"
            step="0.5"
            className="w-full border border-border rounded-xl px-3 py-2 text-sm text-text-dark outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 bg-surface transition-all duration-150 placeholder:text-text-faint"
      ...
      <TopicSubtopicSelect
        ...
        topics={topics}
        onChange={({ topicId, topicName, subtopicId, subtopicName }) =>
          setQ((p) => ({ ...p, topicId, topicName, subtopicId, subtopicName }))
        }
      />
      ...
      <div className="border-t border-border pt-4">
        <button
          ...
          className="text-xs font-semibold text-text-muted hover:text-accent flex items-center gap-1.5 transition-colors duration-150"
        >
        ...
        {showExplanation && (
          <div className="mt-3 space-y-2 pl-4 border-l-2 border-accent/20">
            <textarea
              rows={2}
              className="w-full border border-border rounded-xl px-3 py-2.5 text-sm text-text-dark outline-none focus:border-accent focus:ring-2 focus:ring-accent/10 resize-none bg-surface transition-all duration-150 placeholder:text-text-faint"
      ...
      <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-border">
        <button
          ...
          className="text-sm font-medium bg-primary hover:bg-primary-hover text-white px-5 py-2 rounded-xl transition-all duration-150 shadow-sm disabled:opacity-40 disabled:cursor-not-allowed hover:shadow-md active:scale-[0.98]"
        >
        ...
        <button
          type="button"
          onClick={onCancel}
          className="text-sm text-text-muted hover:text-text-dark px-3 py-2 rounded-xl hover:bg-background transition-all duration-150"
        >
          Cancel
        </button>
        {!isValid && (
...
// ─── GoogleFormPanel, UrlExtractor, JsonPaster, ImportPreview ─────────────────
// (all unchanged — paste from your original file)
// Only update mapGFormJSON and mapExtractedData below ↓
...
function UrlExtractor({ onImport, hasExistingQuestions }) {
  ...
  async function handleExtract() {
    ...
    try {
      const { extractFromGoogleForm } = await import(
        "../../../services/googleForm.service"
      );
          ...
          {loading && (
            <div className="bg-sky-bg border border-sky/20 rounded-2xl p-4">
              ...
              <div className="w-full bg-sky/15 rounded-full h-1.5 overflow-hidden">
                <div
                  className="h-full bg-sky rounded-full animate-pulse"
                  style={{ width: "60%" }}
                />
              </div>
...
function mapGFormJSON(data) {
  const mcq = data.questions.filter(
    (q) =>
      q.type !== "open-ended" &&
      q.options &&
    ...
    .map((s) => ({
      ...
      questions: mcq
        ...
        .map((q, idx) => ({
          ...
          imageSize: q.text?.trim().replace(/^\d+\.?\s*$/, "")
            ? "medium"
            : "full",
          imageUploading: false,
          options: q.options,
...
export default function SectionsStep({
  ...
  const [bulkSubtopicName, setBulkSubtopicName] = useState("");
  const [bulkSubLoading, setBulkSubLoading] = useState(false);

  // Load topics when grade is available
  useEffect(() => {
    if (!grade) {
      setTopics([]);
      return;
  ...
  // Load subtopics for the bulk-assign panel when bulk topic changes
  useEffect(() => {
    if (!bulkTopicId) {
      setBulkSubtopics([]);
      setBulkSubtopicId("");
    ...
    getSubtopics(bulkTopicId)
      .then((subs) => {
        setBulkSubtopics(subs);
        setBulkSubtopicId("");
        setBulkSubtopicName("");
      })
      .catch(console.error)
  ...
  const selectAll = () =>
    setSelectedIds(new Set(activeSection.questions.map((q) => q.id)));
        ...
        questions: s.questions.map((q) =>
          selectedIds.has(q.id)
            ? {
                ...
                subtopicId: bulkSubtopicId,
                subtopicName: bulkSubtopicName,
              }
            : q
        ),
      }))
      ...
      <div className="w-full md:w-48 flex-shrink-0">
        ...
        <div className="flex md:flex-col gap-2 overflow-x-auto pb-1 md:pb-0">
          {sections.map((s) => {
            ...
            return (
              <button
                ...
                className={`text-left px-3 py-2.5 rounded-xl text-sm transition-all duration-150 flex-shrink-0 ${
                  active
          ...
          <button
            type="button"
            onClick={addSection}
            className="text-left px-3 py-2.5 rounded-xl text-sm border border-dashed border-border text-text-faint hover:border-accent hover:text-accent hover:bg-accent/3 transition-all duration-150 flex-shrink-0"
          >
            + Add
          </button>
        </div>
      ...
      {activeSection && (
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-5">
            <input
              className={`border rounded-xl px-3 py-2 text-sm font-semibold outline-none bg-surface flex-1 min-w-[140px] max-w-xs transition-all duration-150 ${
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <input
                ...
                min="0"
                step="0.5"
                className="w-16 border border-border rounded-xl px-2 py-2 text-sm text-text-dark outline-none text-center bg-surface focus:border-accent focus:ring-2 focus:ring-accent/10 transition-all duration-150"
            ...
            {sections.length > 1 && (
              <button
                ...
                className="text-xs text-danger/60 hover:text-danger hover:bg-danger-bg px-2 py-1 rounded-lg transition-all duration-150 flex-shrink-0"
              >
                Delete section
              </button>
            )}
          ...
          <div className="flex gap-0 mb-5 border border-border rounded-lg p-0.5 bg-background w-fit">
            {[
              ...
              <button
                ...
                className={`text-xs px-4 py-1.5 rounded-[7px] font-medium transition-all duration-150 whitespace-nowrap ${
                  importTab === k
                    ? "bg-surface text-text-dark shadow-sm"
                    : "text-text-muted hover:text-text-dark"
                }`}
          ...
          {importTab === "gform" ? (
            ...
            <>
              {/* ── Bulk-assign toolbar (visible in select mode) ── */}
              {selectMode && (
                <div className="mb-4 border border-violet-200 bg-violet-50 rounded-2xl p-4 space-y-3">
                  {/* Header row */}
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-semibold text-violet-700">
                        {selectedIds.size} of {activeSection.questions.length}{" "}
                        selected
                      </span>
                      <button
                        type="button"
                    ...
                    <button
                      ...
                      className="text-xs text-text-muted hover:text-text-dark transition-colors"
                    >
                      ✕ Cancel
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[11px] font-semibold text-violet-600 uppercase tracking-widest mb-1 block">
                        Topic
                      </label>
                      <select
                        className={selectCls}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        disabled={!bulkTopicId || bulkSubLoading}
                        onChange={(e) => {
                          const id = e.target.value;
                          setBulkSubtopicId(id);
                          setBulkSubtopicName(
                            bulkSubtopics.find((s) => s.id === id)?.name || ""
                          );
                        }}
                      >
                        <option value="">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <button
                      ...
                      disabled={!bulkTopicId || selectedIds.size === 0}
                      className="text-xs font-semibold bg-violet-600 hover:bg-violet-700 text-white px-4 py-2 rounded-xl transition-all duration-150 shadow-sm disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      Apply to {selectedIds.size} question
                      {selectedIds.size !== 1 ? "s" : ""}
                    </button>
                    <button
                      type="button"
                      onClick={clearBulkTopic}
                      disabled={selectedIds.size === 0}
                      className="text-xs font-medium text-danger/70 hover:text-danger hover:bg-danger-bg border border-danger/20 px-3 py-2 rounded-xl transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      Clear topic from selected
                    </button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2 mb-3">
                {activeSection.questions.map((q, i) => (
                  <div
                    key={q.id}
                    className={`flex items-start gap-3 border rounded-xl px-3 py-3 bg-surface group transition-all duration-150 ${
                      selectMode && selectedIds.has(q.id)
                  ...
                  >
                    ...
                    {/* Index */}
                    <span className="text-[11px] font-bold text-text-faint w-6 pt-0.5 flex-shrink-0">
                      Q{i + 1}
                    </span>
                    ...
                    <div
                      ...
                      onClick={() => selectMode && toggleSelectQ(q.id)}
                    ...
                    >
                      {q.imageUrl && !q.text && (
                        <img
                          src={q.imageUrl}
                          alt=""
                          className="h-14 rounded-lg mb-1.5 object-contain"
                        />
                      )}
                      ...
                      <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                        ...
                        <span className="text-[11px] text-text-faint bg-background border border-border px-1.5 py-0.5 rounded-full">
                        ...
                        {q.tags?.length > 0 && (
                          <span className="text-[10px] font-medium text-indigo bg-indigo-bg border border-indigo/15 px-1.5 py-0.5 rounded-full">
                            {q.tags.join(", ")}
                          </span>
                        )}
                        {/* ── Topic / Subtopic badge ── */}
                        {q.topicName && (
                ...
                {activeSection.questions.length === 0 && !showForm && (
                  <div className="border-2 border-dashed border-border rounded-2xl py-12 text-center bg-background">
              ...
              {!showForm && (
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    ...
                    className="flex-1 sm:flex-none text-sm font-medium border-2 border-dashed border-accent/30 text-accent px-4 py-3 rounded-xl hover:bg-accent/5 hover:border-accent/50 transition-all duration-150 flex items-center justify-center gap-2"
                  >
                    + Add Question
                  </button>

                  {/* Bulk assign button — only shown when topics exist and there are questions */}
                  {topics.length > 0 && activeSection.questions.length > 0 && (
                    <button
                      type="button"
                      onClick={() => {
                        exitSelectMode();
                      ...
                      className="text-sm font-medium border-2 border-dashed border-violet-300 text-violet-600 px-4 py-3 rounded-xl hover:bg-violet-50 hover:border-violet-400 transition-all duration-150 flex items-center gap-2"
                    >
                      ⊕ Bulk Assign Topic
                    </button>
                  )}

                  {/* Loading indicator while topics fetch */}
                  {topicsLoading && (
